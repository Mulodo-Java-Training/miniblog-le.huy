...
public class UserController
{
    ...
    public Response createUser(@Form @Valid UserSignupForm data)
    {
        ...
        if (user_service.checkUserExist(data.username)) {
            return Response.status(SystemValue.ERRORCODE_2001)
                    .entity(ErrorCodeUtil.set_error_code(SystemValue.ERRORCODE_2001)).build();
        }

        if (user_service.checkEmailExist(data.email)) {
            return Response.status(SystemValue.ERRORCODE_2002)
                    .entity(ErrorCodeUtil.set_error_code(SystemValue.ERRORCODE_2002)).build();
        }

        Users user = data.set_sign_up_data();

        if (!user_service.insertUser(user)) {
            return Response.status(SystemValue.ERRORCODE_2003)
                    .entity(ErrorCodeUtil.set_error_code(SystemValue.ERRORCODE_2003)).build();
        }

        return Response
                .status(SystemValue.CODE_200)
                .entity(ErrorCodeUtil.set_code(SystemValue.CODE_200,
                        "User account was created successfully.", null)).build();
    }
    ...
    public Response loginUser(@Form @Valid UserLoginForm data)
    {
        ...
        if (token == null) {
            return Response.status(SystemValue.ERRORCODE_2004)
                    .entity(ErrorCodeUtil.set_error_code(SystemValue.ERRORCODE_2004)).build();
        }

        return Response.status(SystemValue.CODE_200)
                .entity(ErrorCodeUtil.set_code(SystemValue.CODE_200, "Login successful!", token))
                .build();
    }
            ...
            @Pattern(regexp = SystemValue.ACCESS_TOKEN_STRING_RANGE) @HeaderParam("access_token") String access_token)
    {
        ...
        if (!user_service.logoutUser(token)) {
            return Response.status(SystemValue.ERRORCODE_2005)
                    .entity(ErrorCodeUtil.set_error_code(SystemValue.ERRORCODE_2005)).build();
        }

        return Response.status(SystemValue.CODE_200)
                .entity(ErrorCodeUtil.set_code(SystemValue.CODE_200, "Logout successful!", null))
                .build();
    }
    ...
    public Response updateUser(
            @Pattern(regexp = SystemValue.ACCESS_TOKEN_STRING_RANGE) String access_token,
            @Form @Valid UserUpdateForm data)
    {
        ...
        if (!user_service.updateUserInfo(data, token.getUser().getId())) {
            return Response.status(SystemValue.ERRORCODE_2006)
                    .entity(ErrorCodeUtil.set_error_code(SystemValue.ERRORCODE_2006)).build();
        }
            ...
            @QueryParam("mode") String mode, @QueryParam("id") int id)
    {
        ...
        Response resp = Response.status(SystemValue.CODE_200)
                .entity(ErrorCodeUtil.set_code(SystemValue.CODE_200, null, null)).build();
        ...
        if (mode == SystemValue.USER_GET_MODE_ID) {
            ...
            if (user == null) {
                ...
                resp = Response
                        .status(SystemValue.CODE_200)
                        .entity(ErrorCodeUtil.set_code(SystemValue.CODE_200, "Info of user by id",
                                user)).build();
            }
        }
        return resp;
            ...
            @Form @Valid UserChangePasswordForm data)
    {
        ...
        if (null == token) {
            return Response.status(SystemValue.ERRORCODE_2007)
                    .entity(ErrorCodeUtil.set_error_code(SystemValue.ERRORCODE_2007)).build();
        }

        return Response
                .status(SystemValue.CODE_200)
                .entity(ErrorCodeUtil.set_code(SystemValue.CODE_200, "Password Change successful!",
                        token)).build();
    }
    ...
    public Response searchUserByName(
            @Pattern(regexp = SystemValue.ACCESS_TOKEN_STRING_RANGE) @HeaderParam("access_token") String access_token,
            @Pattern(regexp = SystemValue.USERNAME_SEARCH_RANGE) @PathParam("query") String query)
    {
        ...
        return Response
                .status(SystemValue.CODE_200)
                .entity(ErrorCodeUtil.set_code(SystemValue.CODE_200, "result return",
